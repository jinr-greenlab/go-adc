...
func (s *Server) SetDefaults() {
	if s.Address == "" {
		s.Address = config.DefaultDiscoverAddress
	}
	if s.Port == "" {
		s.Port = config.DefaultDiscoverPort
	}
	if s.IfaceName == "" {
		s.IfaceName = config.DefaultDiscoverIfaceName
	}
}
...
func (s *Server) Run() error {
	...
	go func() {
		...
		for packet := range source.Packets() {
			...
			layer := packet.Layer(layers.LayerTypeLinkLayerDiscoveryInfo).(*layers.LinkLayerDiscoveryInfo)
